...
public class PlayerControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            rb2D.AddForce(Vector2.up * forceAmount); //apply to the up multiplied by the "force" var
        }

        if (Input.GetKey(KeyCode.S)) //if S is pressed
        {
            rb2D.AddForce(Vector2.down * forceAmount); //apply to the up multiplied by the "force" var
        }

        if (Input.GetKey(KeyCode.A)) //if A is pressed
        {
            rb2D.AddForce(Vector2.left * forceAmount); //apply to the up multiplied by the "force" var
        }

        if (Input.GetKey(KeyCode.D)) //if D is pressed
        {
            rb2D.AddForce(Vector2.right * forceAmount); //apply to the up multiplied by the "force" var
        }
    }
}
